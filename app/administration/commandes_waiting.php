									...
									<div class="ibox-fullwidth-block">
										<table class="table table-striped table-hover no-border m-b-0">
											...
											<tbody>
												...
												$Commandes = $db->query('SELECT * FROM hc_orders'); 
										
												while($C = $Commandes->fetch()) {
													$acc = $db->prepare('SELECT * FROM hc_users WHERE sso = ?');
													$acc->execute(array($C['sso']));
													$cc = $acc->fetch();
													if($cc['rank'] == 2) {
														echo '<tr>
															<td class="p-l-20"><a href="invoice.html">Prioritaire - '.$C['id'].'</a></td>
															<td>'.$C['name'].'</td>
															<td>'.$C['extension'].'</td>
															<td>'.$C['solution'].'</td>
															<td>'.$C['cms'].'</td>
															<td>'.$C['emulator'].'</td>
															<td class="p-r-20">'.$this->ConvertTime($C['date']).'</td>
															<td><a href="/administration/commandes/waiting/'.$C['id'].'">Livrer</a></td>
															</tr>';
													}
													else {
														echo '<tr>
