...
if($rowCount == 0) {
	header('Location: /administration/commandes/actives');
} else {
	$fetch = $Recovery->fetch();
	$Access = $db->prepare('SELECT * FROM hc_services_access WHERE name = ? AND extension = ? AND sso = ?');
	$Access->execute(array($fetch['name'], $fetch['extension'], $fetch['sso']));
	$fetchA = $Access->fetch();
}
...
<html>
	...
	<body class="boxed-layout">
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="ibox ibox-fullheight">
								<div class="ibox-head">
									<div class="ibox-title">Modifier du service #<?= $fetch['id']; ?></div>
								</div>
								<div class="ibox-body" style="padding: 35px">
									<div class="ibox-fullwidth-block">
										<form id="hc_order_actives">
											<div class="row">
												<div class="col-md-4">
												...
												<div class="col-md-4 form-group">
													...
													<input class="form-control" type="text" value="<?= $fetchA['mysql_host']; ?>" name="host_mysql" placeholder="Hôte MySQL">
													<label>Utilisateur MySQL</label>
													<input class="form-control" type="text" value="<?= $fetchA['mysql_user']; ?>" name="user_mysql" placeholder="Utilisateur MySQL">
													<label>Mot de passe MySQL</label>
													<input class="form-control" type="text" value="<?= $fetchA['mysql_password']; ?>" name="password_mysql" placeholder="Mot de passe MySQL">
													<label>Base de données MySQL</label>
													<input class="form-control" type="text" value="<?= $fetchA['mysql_database']; ?>" name="database_mysql" placeholder="Hôte MySQL">
												</div>
												<div class="col-md-4 form-group">
													<label>Hôte FTP</label>
													<input class="form-control" type="text" value="<?= $fetchA['ftp_host']; ?>" name="host_ftp" placeholder="Hôte FTP">
													<label>Utilisateur FTP</label>
													<input class="form-control" type="text" value="<?= $fetchA['ftp_user']; ?>" name="user_ftp" placeholder="Utilisateur FTP">
													<label>Mot de passe FTP</label>
													<input class="form-control" type="text" value="<?= $fetchA['ftp_password']; ?>" name="password_ftp" placeholder="Mot de passe FTP">
												</div>
												<div class="col-md-12">
										...
										<button id="hc_order_suspend" style="margin-top: 10px" class="btn btn-danger btn-block">Suspendre</button>
									</div>
								</div>
							</div>
						</div>
		...
		<script>var name = "<?= $fetch['name']; ?>"; var extension = "<?= $fetch['extension']; ?>"; var sso = "<?= $fetch['sso']; ?>"</script>
