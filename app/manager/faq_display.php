...
$Recovery = $db->prepare('SELECT * FROM hc_faq WHERE id = ?');
$Recovery->execute(array($get_id));
$rowCount = $Recovery->rowCount();

if($rowCount == 0) {
	header('Location: /Client/FAQ');
} else {
									...
									<section class="hc_box_section">
										<div class="hc_section_card">
											<div class="hc_new_offers_box">
												<h3 class="hc_panel_box_title"><?= $fetch['title']; ?></h3>
												<p><?= $fetch['content']; ?></p>
											</div>						
										</div>
									</section>
									...
									<section class="hc_box_section">
										<div class="hc_section_card">
											<div class="hc_panel_box">
												<h3 class="hc_panel_box_title">Listes des questions</h3>
												...
												<ul>
													...
													$FAQ = $db->query('SELECT * FROM hc_faq');
													
													$row = $FAQ->rowCount();
													
													if($row == 0) {
														echo 'Aucune question n\'a été posté';
													} else {
													
													while($f = $FAQ->fetch()) {
														echo '<li>
														<a href="/Client/FAQ/'.$f['id'].'">
															<span class="hc_card_icons"><i class="fa fa-circle"></i></span>
															<span class="hc_card_content">'.$f['title'].'</span>
														</a>
													</li>';
													}
													}
