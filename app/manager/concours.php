    ...
    <body>
					...
					<div id="main-wrapper">
						...
						<div class="row hc_container">
							<div style="margin-top: -20px">
								...
								$rc = $db->query('SELECT * FROM hc_concours');
								...
								<div class="col-md-6">
									<div class="hc_panel">
										<div class="hc_panel_title_sondage">
											<i class="fa fa-star"></i> <?= $r['id']; ?> - Concours VIP
										</div>
										<div class="hc_panel_body">
											<div class="hc_panel_bloc">
												<div class="hc_panel_info">
													<?= $r['content']; ?>
													<hr>
													<?php if($r['end_date'] >= date('Y-m-d H:i:s')) { ?>
													<button name="partic" id="<?= $r['id']; ?>" class="btn btn-primary btn-block">Je participe</button>
													<?php } else { ?>
		...
		<script src="<?= $this->App('url'); ?>/app/ajax/notifications.js"></script>
			...
			$(document).ready(function() {
			 $('[name="partic"]').on('click', function(e) {
				 e.preventDefault();
				 ...
				 $.ajax({
					 ...
					 {
						 if(data == 'success') {
							 $('#error').html('<div class="col-md-12"><div class="alert bg-success">Vous participez au concours</div></div>');
						 } else {
