				...
				<div class="page-inner">
					<div id="main-wrapper">
						...
						<div class="row hc_container">
							...
							$Services = $db->prepare('SELECT * FROM hc_services WHERE sso = ?');
							$Services->execute(array($_SESSION['account']['sso']));
							$rowCount = $Services->rowCount();
												
							if($rowCount == 0) {
								echo '<div class="col-md-12">Vous n\'avez aucun service.</div>';
							} else {
									...
									<section class="hc_box_section">
										<div class="hc_section_card">
											<div class="hc_new_offers_box">
												<h3 class="hc_new_offers_box_title">Pack <?= $S['solution']; ?></h3>
												<div class="hc_pack_name"><?= $S['name'].$S['extension']; ?></div>
												<span class="hc_pack_<?= $S['status']; ?>">Pack <?= $this->StatusServices($S['status']); ?></span>
												<center><a href="/Client/Services/<?= $S['id']; ?>" class="btn btn-primary btn-block">Administrer</a></center>
											</div>						
										</div>
									</section>
								</div>
